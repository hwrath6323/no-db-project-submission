...
app.get('/album-info/:id', (req, res) => {
    axios.get(`https://api.discogs.com/releases/${req.params.id}?key=${process.env.DISCOG_APIKEY}&secret=${process.env.DISCOG_APISECRET}`)
        .then(response => {
            console.log(response.data)
            res.send(response.data);
        })
        .catch(err => {
            res.status(500).send({message: 'you killed it'})
        });
})
                            // full CRUD? (4)
